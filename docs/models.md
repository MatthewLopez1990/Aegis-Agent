...
- Live chat completion calls for OpenAI, Anthropic, Google Gemini, Mistral, Cohere, OpenRouter, AWS Bedrock through verified AWS CLI cloud identity, Ollama, LM Studio, configured Azure Foundry/OpenAI v1 routes, and configured custom OpenAI-compatible routes. AWS Bedrock, LM Studio, and Azure Foundry accept arbitrary deployment IDs after their provider prefix.
