...
- Live chat completion calls for OpenAI, Anthropic, Google Gemini API key or configured Vertex AI cloud identity, Mistral, Cohere, OpenRouter, AWS Bedrock through verified AWS CLI cloud identity, Ollama, LM Studio, configured Qwen Code Coding Plan subscription, configured Azure Foundry/OpenAI v1 routes through API key or verified Azure CLI cloud identity, and configured custom OpenAI-compatible routes. Google, Qwen, AWS Bedrock, LM Studio, and Azure Foundry accept arbitrary deployment IDs after their provider prefix.
