...
By default, `aegis tui` opens a curses-backed fullscreen command deck with the Aegis shield identity in the header. Use Tab or Right/Left to move focus between panels, Up/Down to select rows inside the focused panel, Enter to open the selected row, `/` to run slash commands in-place, any printable key to start typing a command, `r` to refresh, `?` for help, and `q` or Esc to exit. Navigation rows such as Setup, Tools, Memory, Logs, Tasks, and Approvals drill into submenus; command rows dispatch through the existing governed TUI command path and render output in the detail pane. Mouse clicks select and open rows when the terminal supports curses mouse events. Non-interactive shells or `--classic` fall back to the line-oriented command deck.
