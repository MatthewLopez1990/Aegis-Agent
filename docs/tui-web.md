...
- `agents [status|profiles|profile-create <name>|profile-disable <id>|delegate <role> <task> [--approved]|handoff <card-id> <lane> [reason]|run <card-id> --approved|run-batch --approved [--limit n] [--card-id id]|review-packet <card-id>|verify-packet <packet-id-or-path>]`
