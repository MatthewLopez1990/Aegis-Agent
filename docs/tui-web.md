...
`remote-control relay-notify --pairing-id <id> --relay-auth-secret <name> --approved [--event task-updated] [--task-id <id>]` publishes one metadata-only notification envelope to the registered relay for mobile or gateway clients. The payload excludes pairing tokens, relay bearer values, raw user requests, plans, and receipts. Failed notification attempts persist in `.aegis/remote_control_pairings.json` as metadata-only outbox rows, and `remote-control relay-retry --pairing-id <id> --relay-auth-secret <name> --approved` retries pending or failed rows without exposing relay secrets.
