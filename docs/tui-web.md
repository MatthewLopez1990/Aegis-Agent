...
By default, `aegis tui` opens a curses-backed fullscreen command deck. Use Tab or Right/Left to move focus between panels, Up/Down to select rows inside the focused panel, Enter to open the selected item through the existing governed command path, `/` to run any TUI command in-place, `r` to refresh, `?` for help, and `q` or Esc to exit. Mouse clicks select panels when the terminal supports curses mouse events. Non-interactive shells or `--classic` fall back to the line-oriented command deck.
