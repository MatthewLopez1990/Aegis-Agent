...
- `agents [status|autonomy-preflight|profiles|profile-create <name>|profile-disable <id>|delegate <role> <task> [--approved]|handoff <card-id> <lane> [reason]|run <card-id> --approved|run-batch --approved [--limit n] [--card-id id]|review-packet <card-id>|verify-packet <packet-id-or-path>|model-review <card-id> --approved]`
...
- `tools [list|run|enable|disable]`, `allowed-tools`, `bashes|processes [list|start|stop|logs]`
- `toolsets`
