...
The terminal deck renders with a neon command-center skin inspired by the Aegis shield identity: a large ASCII wordmark, cyan/purple command framing, navigation tabs, status flag panels, overview runtime cards, and boxed command lanes. `/setup` now opens a guided setup tour instead of dumping only raw JSON; the tour keeps advanced detail hidden behind submenus such as `/setup initialize`, `/setup model-auth`, `/setup connectors`, `/setup backends`, `/setup remote-control`, `/setup interfaces`, and `/setup verify`. `/setup json` returns the original machine-readable readiness packet for automation and tests.
