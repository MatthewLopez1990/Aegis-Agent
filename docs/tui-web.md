...
`remote-control relay-notify --pairing-id <id> --relay-auth-secret <name> --approved [--event task-updated] [--task-id <id>]` publishes one metadata-only notification envelope to the registered relay for mobile or gateway clients. The payload includes a stable delivery id/idempotency key and excludes pairing tokens, relay bearer values, raw user requests, plans, and receipts. Failed notification attempts persist in `.aegis/remote_control_pairings.json` as metadata-only outbox rows, successful attempts store only a whitelisted redacted relay receipt, and `remote-control relay-retry --pairing-id <id> --relay-auth-secret <name> --approved` retries due pending or failed rows without exposing relay secrets.
