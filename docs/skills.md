...
Local plugin manifests can group skills with MCP servers and lifecycle hooks without bypassing those controls. Use `plugin install /path/to/plugin.json` or `plugins install /path/to/plugin.json --unsigned-local` for development-only unsigned skill manifests, then `plugin enable|disable|remove <plugin_id>` or the matching token-protected `/plugins` API endpoints. `plugins marketplace` and `plugins updates` provide metadata-only marketplace discovery and update planning from the built-in catalog or an operator-supplied local catalog JSON. `plugins fetch-manifest <plugin_id>` can download one allowlisted HTTPS marketplace manifest for review only when the catalog supplies a matching SHA-256; `plugins fetch-bundle <plugin_id>` can download one allowlisted HTTPS bundle only when the catalog SHA-256 matches and a brokered HMAC signature verifies; `plugins install-bundle <plugin_id>` verifies the signed bundle, extracts a JSON plugin manifest into private review state, and installs through the governed local plugin lifecycle; `plugins install-marketplace <plugin_id>` performs manifest verification and then installs through the same lifecycle; `plugins update-marketplace <plugin_id> --approved` backs up the current manifest before applying a newer SHA-verified marketplace manifest update. `plugins prepare-update <plugin_id>` writes a private SHA-verified update candidate for review, and `plugins apply-prepared-update <candidate_id> --approved` revalidates that candidate before applying it. These paths avoid dynamic plugin-code import, marketplace token capture, unattended remote bundle auto-install, and unattended unsigned auto-update. Plugin resource paths must stay relative to the plugin manifest directory, duplicate skills or MCP server names are rejected, owned resources are removed with the plugin, and failed installs roll back any resources registered before the failure. Marketplace entries still require explicit review, explicit install, explicit direct-update approval, or explicit prepared-candidate approval through the governed manifest path.
