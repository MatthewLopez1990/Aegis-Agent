...
- Weak multi-agent isolation: approved subagent work is visible through durable delegation cards with tainted-instruction metadata, but the current runtime does not implement recursive autonomous subagents; the model is a controlled root orchestrator design.
