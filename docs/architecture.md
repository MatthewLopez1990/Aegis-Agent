...
- Weak multi-agent isolation: approved subagent work is visible through durable delegation cards with tainted-instruction metadata, durable profile and budget snapshots, sanitized handoff receipts, deterministic isolated worker-run receipts, model-ready review packets, approved sanitized model-review receipts, and approved scoped autonomy step-plan receipts, but the current runtime does not implement recursive autonomous model-loop subagents; the model is a controlled root orchestrator design.
