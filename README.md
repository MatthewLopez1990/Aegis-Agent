...
- Approval responses include portable action hints and chat-style utterances such as `approve`, `yes proceed`, `deny`, `no do not do that`, and `let's revert`; inbound channel events can record those replies as non-executing approval intents, so future Slack/Discord adapters can keep payload-matched safeguards while making operator decisions quick.
...
- Live connector promotion is tracked with a dashboard and TUI readiness checklist covering credential handles, network allowlists, explicit live-enable flags, human approval, receipt redaction, mock fallback, read-surface inventory, and promotion scope before real service writes are enabled.
