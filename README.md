...
- Browser and media artifact flows now emit private local artifacts, SHA-256 receipts, sandbox metadata, redacted artifact-facing session fields, and subprocess-isolated local media artifacts; the dashboard separates completed browser/media hardening controls from deeper work such as real browser automation boundaries and provider-backed media workers with stricter OS-level sandboxing.
