...
- Marketplace plugin install and update application are explicit and SHA-verified from allowlisted HTTPS manifests. Signed bundle fetch is available for private review only with SHA-256 plus brokered HMAC verification. Remote bundle auto-install, dynamic plugin imports, marketplace token capture, unattended unsigned auto-update, and trusting a marketplace signing chain remain blocked.
