...
- Curses-backed selectable TUI command deck with a compact neon three-column startup control center, restored Aegis shield identity header, keyboard/mouse panel focus, Enter-to-open submenus, selectable dashboard/setup/tool/memory/approval items, a detail/output pane, live `/` slash-command dispatch, nested `menu operate|govern|setup|build|explore` views, guided `/setup next` onboarding, hidden setup submenus, and a `--classic` fallback line-oriented deck.
