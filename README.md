...
- Curses-backed Hermes-style TUI with a restored Aegis shield identity header, prompt-first composer, live `/` slash-command palette, selectable palette rows, command history, Tab completion, guided `/setup next` onboarding through slash commands, and a `--classic` fallback line-oriented deck.
...
- Built-in governed tool catalog with 74 policy-visible tools covering browser, web, files, shell, memory, messaging, media, voice, video, subagent, research, and MCP capabilities.
- Enterprise readiness snapshots through `aegis enterprise-readiness`, dashboard `enterprise_readiness`, memory health scoring, self-improvement readiness blockers, and a prompt-first TUI with the restored Aegis shield identity plus active audit/session/approval/model/workspace flags.
