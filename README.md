...
- Approval responses include portable action hints and chat-style utterances such as `approve`, `yes proceed`, `deny`, `no do not do that`, and `let's revert`, so future Slack/Discord adapters can keep payload-matched safeguards while making operator decisions quick.
