...
- Dependency-free TUI command deck and browser GUI for runtime control, task resume/cancel, security posture, approvals, recent tasks, session-linked task recovery, models, channels, tools, schedules, work boards, and audit evidence.
...
- Browser and media artifact flows now emit private local artifacts, SHA-256 receipts, sandbox metadata, redacted artifact-facing session fields, subprocess-isolated local media artifacts with POSIX resource limits where supported, and opt-in provider-backed media artifacts through allowlisted HTTPS APIs with brokered tokens; the dashboard separates completed browser/media hardening controls from deeper work such as real browser automation boundaries and provider-specific media adapters with stricter platform sandboxing.
