...
- Scheduler with review activation and governed run-due execution, session history, Kanban work boards, approval-gated subagent delegation queue/status/profile/budget/handoff/run/batch-run/review-packet/autonomy-step/autonomy-preflight receipt surfaces with deterministic isolated worker subprocesses and parent-bound sanitized review receipts, governed stdio MCP calls, local lifecycle hooks with approval defaults and redacted receipts, governed local plugin lifecycle for skills/MCP/hooks, SHA-verified marketplace manifest fetch/install, SOUL/context-file loader, and dry-run migration inspection.
