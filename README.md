...
- Live OpenAI, Anthropic, Google Gemini, Mistral, Cohere, OpenRouter, Nous, DeepSeek, xAI, Kimi, MiniMax, Z.AI, Qwen, Ollama, LM Studio, and custom OpenAI-compatible model invocation through the local secrets broker, plus model auth login, guarded provider-native handoff targets for Copilot/Google Cloud/Qwen/Bedrock/Azure, and verified Codex/Claude Code subscription CLI bridges that avoid token import.
