...
- Native mobile push delivery and broad cloud relay delivery remain blocked until explicit push/gateway transport, brokered relay auth, origin allowlists, revocation propagation, and end-to-end mobile/gateway lifecycle controls are configured; the current HTTPS relay notification path records durable redacted delivery receipts but is not a broad cloud relay service.
