...
class GitLabConnectorStub(MockServiceConnector):
    ...
    def write(self, request: ConnectorRequest) -> ConnectorResult:
        ...
        if not request.approved:
            return ConnectorResult(self.spec.name, request.operation, False, {}, error="GitLab live write requires approval")
        if not self.live_writes:
            return ConnectorResult(self.spec.name, request.operation, False, {}, error="GitLab live writes are disabled")
