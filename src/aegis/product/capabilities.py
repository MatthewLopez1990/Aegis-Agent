...
def _live_gap_backlog(
    ...
    return [
        ...
        },
        {
            ...
            "detail": "Sanitized browser rendering, bounded static DOM snapshots, approved static form fills, approved static GET form submits, and approved static-anchor navigation are available for stored HTTP-content sessions; provider-backed media artifacts and transcription can run through allowlisted HTTPS adapters, including OpenAI-style image JSON, multipart image edit, TTS, and audio transcription adapters, while real page automation and broader provider-specific media depth still require stronger sandboxing.",
...
def _browser_media_operator_checklist(implemented_controls: list[str], remaining_depth_work: list[str]) -> list[dict[str, str]]:
    ...
    return [
        ...
        },
        {
            ...
            "detail": "Provider-backed image, TTS, and transcription paths exist, including OpenAI-style image JSON, multipart image edit, TTS, and audio transcription adapters; provider-specific video and additional image/audio adapters still need expansion.",
        },
        {
