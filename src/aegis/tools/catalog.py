...
def default_tool_specs() -> tuple[ToolSpec, ...]:
    return (
        ...
        ToolSpec("browser", "Dependency-light HTTP-content browser session with text/table extraction, sanitized rendered snapshots, virtual interaction records, approved read-only live snapshots, and opt-in approved live selector mutation.", "read/write", RiskLevel.HIGH, {"action": "string"}, {"result": "object"}, implementation_status="local_sandbox_or_opt_in_live", approval_required=True, categories=("browser",)),
