...
def default_tool_specs() -> tuple[ToolSpec, ...]:
    return (
        ...
        ToolSpec("browser", "Dependency-light HTTP-content browser session with text/table extraction, sanitized rendered snapshots, virtual interaction records, approved read-only live snapshots, opt-in approved live selector mutation, private live downloads, workspace-scoped live uploads, and bounded redacted live JavaScript evaluation.", "read/write", RiskLevel.HIGH, {"action": "string"}, {"result": "object"}, implementation_status="local_sandbox_or_opt_in_live", approval_required=True, categories=("browser",)),
        ...
        ToolSpec("image_generate", "Creates an approved deterministic local PNG preview artifact or an approved allowlisted provider-backed image artifact, including OpenAI-style image JSON, Stability AI v1 text-to-image, and Google Vertex Imagen predict adapters.", "write", RiskLevel.HIGH, {"prompt": "string", "provider_url": "string", "token_secret": "string", "provider_adapter": "string"}, {"asset_path": "string"}, implementation_status="allowlisted_live_or_local", approval_required=True, categories=("media",)),
        ToolSpec("tts", "Creates an approved local WAV audio cue artifact or an approved allowlisted provider-backed audio artifact, including OpenAI-style TTS, ElevenLabs TTS, speech-to-speech, and text-to-dialogue adapters.", "write", RiskLevel.MEDIUM, {"text": "string", "audio_path": "string", "inputs": "array", "provider_url": "string", "token_secret": "string", "provider_adapter": "string"}, {"asset_path": "string"}, implementation_status="allowlisted_live_or_local", approval_required=True, categories=("voice",)),
        ToolSpec("voice_transcribe", "Transcribes local audio text fallbacks or an approved allowlisted provider-backed audio upload, including OpenAI-style and ElevenLabs speech-to-text adapters.", "read", RiskLevel.MEDIUM, {"audio_path": "string", "provider_url": "string", "token_secret": "string", "provider_adapter": "string"}, {"text": "string"}, implementation_status="allowlisted_live_or_local", categories=("voice",)),
        ...
        ToolSpec("video_generate", "Submits and manages approved allowlisted provider-backed video generation jobs, including an OpenAI-style video adapter with status, download, and delete actions.", "write", RiskLevel.HIGH, {"prompt": "string", "provider_url": "string", "token_secret": "string", "provider_adapter": "string", "action": "string", "video_id": "string"}, {"status": "string"}, implementation_status="allowlisted_live_or_local", approval_required=True, categories=("video", "media")),
        ...
        ToolSpec("browser_click", "Records an approved virtual click selector, follows an exact static anchor through the governed HTTP connector, or runs an opt-in approved live selector click when live=true.", "write", RiskLevel.HIGH, {"selector": "string"}, {"status": "string"}, implementation_status="local_sandbox_or_opt_in_live", approval_required=True, categories=("browser",)),
        ToolSpec("browser_fill", "Records approved form state, updates matching stored static controls, or runs an opt-in approved live selector fill when live=true.", "write", RiskLevel.HIGH, {"fields": "array"}, {"status": "string"}, implementation_status="local_sandbox_or_opt_in_live", approval_required=True, categories=("browser",)),
        ToolSpec("browser_submit", "Submits one approved static GET form through the governed HTTP connector, or runs an opt-in approved live form submit when live=true.", "write", RiskLevel.HIGH, {"session_id": "string", "selector": "string"}, {"status": "string"}, implementation_status="local_sandbox_or_opt_in_live", approval_required=True, categories=("browser",)),
        ToolSpec("browser_screenshot", "Writes a deterministic local PNG snapshot of browser session state or an opt-in approved read-only live Chromium PNG when live=true.", "read", RiskLevel.MEDIUM, {"path": "string"}, {"asset_path": "string"}, implementation_status="local_or_opt_in_live_png_snapshot", categories=("browser",)),
        ToolSpec("browser_render_screenshot", "Writes a sanitized Chrome-rendered PNG artifact from stored HTTP-content browser state without page JavaScript.", "read", RiskLevel.MEDIUM, {"session_id": "string"}, {"asset_path": "string"}, implementation_status="local_rendered_snapshot", categories=("browser",)),
        ...
        ToolSpec("browser_dom_snapshot", "Returns a bounded redacted static DOM snapshot from stored HTTP content without page JavaScript, cookies, storage, or live DOM events.", "read", RiskLevel.MEDIUM, {"session_id": "string", "selector": "string"}, {"dom": "array"}, implementation_status="local_sandbox", categories=("browser",)),
        ...
        ToolSpec("code_execute", "Runs approved local Python code with isolated mode, a private working directory, and minimal environment.", "execute", RiskLevel.HIGH, {"language": "string", "code": "string"}, {"stdout": "string"}, approval_required=True, categories=("code",)),
        ToolSpec("python_repl", "Runs an approved local Python snippet with isolated mode, a private working directory, and minimal environment.", "execute", RiskLevel.HIGH, {"code": "string"}, {"stdout": "string"}, approval_required=True, categories=("code",)),
        ...
        ToolSpec("github_pr", "Reads GitHub pull requests and PR comments, turns review comments into governed autofix plans, applies approved local autofix diffs, and can post approved brokered-token comments or autofix responses when configured.", "read/write", RiskLevel.HIGH, {"operation": "string", "provider_url": "string"}, {"result": "object"}, implementation_status="allowlisted_live_read_write_or_mock_connector", approval_required=True, categories=("github",)),
        ...
        ToolSpec("message_send", "Sends or rolls back messages through approved mock summaries or brokered-token live messaging writes when configured.", "write", RiskLevel.HIGH, {"operation": "string", "message": "object", "provider_url": "string"}, {"message_id": "string"}, implementation_status="allowlisted_live_write_or_mock_connector", approval_required=True, categories=("messaging",)),
        ...
        ToolSpec("image_edit", "Creates an approved deterministic local PNG edit-preview artifact or an approved allowlisted provider-backed image edit artifact, including OpenAI-style multipart, Google Imagen edit, Google Imagen upscale, and Google Imagen product background-swap adapters.", "write", RiskLevel.HIGH, {"prompt": "string", "source_path": "string", "mask_path": "string", "provider_url": "string", "token_secret": "string", "provider_adapter": "string"}, {"asset_path": "string"}, implementation_status="allowlisted_live_or_local", approval_required=True, categories=("media",)),
        ...
        ToolSpec("hosted_sandbox_exec", "Submits an approved command to a configured hosted sandbox backend and can run approved lifecycle actions for status, logs, cancellation, artifact download, and rollback.", "execute", RiskLevel.HIGH, {"backend": "string", "command": "string", "action": "string", "job_id": "string", "provider_url": "string"}, {"status": "string"}, implementation_status="backend_gate", approval_required=True, categories=("execution",)),
        ...
        ToolSpec("voice_clone", "Creates an approved provider-backed instant voice clone from workspace-scoped audio samples, including an ElevenLabs IVC adapter with brokered xi-api-key credentials and redacted receipts.", "write", RiskLevel.HIGH, {"name": "string", "audio_paths": "array", "provider_url": "string", "token_secret": "string", "provider_adapter": "string"}, {"voice_id": "string"}, implementation_status="allowlisted_live_provider", approval_required=True, categories=("voice", "media")),
