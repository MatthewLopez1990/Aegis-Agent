...
PLUGIN_COMMANDS = ("list", "install", "enable", "disable", "remove", "reload", "marketplace", "updates", "fetch-manifest", "fetch-bundle", "install-marketplace", "update-marketplace")
...
class AegisTui(cmd.Cmd):
    ...
    def do_plugins(self, arg: str) -> None:
        """plugins list|install|enable|disable|remove|reload|marketplace|updates|fetch-manifest|fetch-bundle|install-marketplace|update-marketplace -- manage governed local plugins."""
        try:
            parts = shlex.split(arg) if arg else []
        except ValueError as exc:
        ...
        try:
            ...
            return
        print("usage: plugins list | plugins install <plugin.json> [--enable] [--unsigned-local] | plugins enable|disable|remove <plugin_id> | plugins reload | plugins marketplace [--query q] [--catalog-path file] | plugins updates [--catalog-path file] | plugins fetch-manifest <plugin_id> [--catalog-path file] | plugins fetch-bundle <plugin_id> [--catalog-path file] [--key-name name] | plugins install-marketplace <plugin_id> [--catalog-path file] [--enable] | plugins update-marketplace <plugin_id> [--catalog-path file] [--enable|--disable] [--force]")
...
def _command_reference() -> str:
    ...
    return "\n".join(
        (
            ...
            "plugins list|install|enable|disable|remove|reload|marketplace|updates|fetch-manifest|fetch-bundle|install-marketplace|update-marketplace",
