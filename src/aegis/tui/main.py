    ...
    def _render_dashboard(self) -> str:
        ...
        lines = [
            ...
            _section(
                "Live Gap Backlog",
                [
                    f"{item['area']}: {item['status']} - gates {', '.join(item.get('verification_gates', [])[:3]) or 'none'} - evals {', '.join(item.get('evaluation_scenarios', [])[:2]) or 'none'} - live adapters {', '.join(adapter.get('name', 'unknown') for adapter in item.get('implemented_live_adapters', [])[:3]) or 'none'} - backend adapters {', '.join(adapter.get('name', 'unknown') for adapter in item.get('implemented_backend_adapters', [])[:3]) or 'none'}"
