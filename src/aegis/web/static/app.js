...
const refresh = async () => {
  try {
    ...
    setList("live-gap-backlog", dashboard.live_gap_backlog || [], (x) => ({
      title: x.area,
      detail: `${x.detail} Live reads: ${(x.live_read_surfaces || []).slice(0, 6).join(", ") || "none"}. Live adapters: ${(x.implemented_live_adapters || []).slice(0, 6).map((adapter) => adapter.name).join(", ") || "none"}. Available adapters: ${(x.available_live_adapters || []).slice(0, 6).map((adapter) => adapter.name).join(", ") || "none"}. Backend adapters: ${(x.implemented_backend_adapters || []).slice(0, 6).map((adapter) => adapter.name).join(", ") || "none"}. Available backends: ${(x.available_backend_adapters || []).slice(0, 6).map((adapter) => adapter.name).join(", ") || "none"}. Hardened: ${(x.implemented_hardening_controls || []).slice(0, 6).map((control) => control.control).join(", ") || "none"}. Remaining depth: ${(x.remaining_depth_work || []).slice(0, 6).join(", ") || "none"}. Controls: ${(x.required_controls || []).join(", ") || "none"}. Gates: ${(x.verification_gates || []).join(", ") || "none"}. Evaluations: ${(x.evaluation_scenarios || []).join(", ") || "none"}. Next: ${(x.next_steps || []).slice(0, 2).join(" ")}`,
