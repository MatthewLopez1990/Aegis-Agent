...
class TuiTests(unittest.TestCase):
    ...
    def test_tui_dashboard_and_capabilities_show_implementation_readiness(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertIn("Explore  capabilities, connectors, channels, browser", rendered)
    ...
    def test_tui_completes_commands_and_common_subcommands(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertIn("AEGIS SHIELD command groups", help_rendered)
            self.assertIn("[Operate]", help_rendered)
            self.assertIn("[Operate] Shield lane", help_rendered)
            self.assertIn("submit <request>", help_rendered)
            self.assertIn("start a governed task", help_rendered)
            self.assertIn("[Govern]", help_rendered)
            self.assertIn("approve|deny <id>", help_rendered)
