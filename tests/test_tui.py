...
class TuiTests(unittest.TestCase):
    ...
    def test_tui_completes_commands_and_common_subcommands(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertIn("/new|/reset|/clear /submit /background|/bg /dashboard", help_rendered)
            self.assertIn("submit <request>", help_rendered)
            self.assertIn("fast [request]", help_rendered)
            self.assertIn("goal|batch|loop", help_rendered)
            ...
            self.assertIn("doctor|config|init", help_rendered)
            ...
            self.assertIn("remote-env|teleport", help_rendered)
            ...
            self.assertIn("mobile/browser relay not yet enabled", slash_rendered)
            ...
            self.assertIn("signed_hook_manifest", alias_commands)
