...
class CliTests(unittest.TestCase):
    ...
    def test_browser_activation_packet_commands_create_and_verify_packet(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(created["packet"]["activation"]["adapter_candidates"][0]["name"], "playwright-chromium")
            self.assertFalse(created["packet"]["activation"]["adapter_candidates"][0]["raw_executable_path_included"])
    ...
    def test_remote_control_relay_command_reports_blocked_preflight(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertTrue(relay_pull["executed_actions"][0]["result"]["metadata_only"])
            self.assertFalse(relay_pull["executed_actions"][0]["result"]["plan_receipt_included"])
            ...
            self.assertTrue(relay_action["result"]["metadata_only"])
            self.assertFalse(relay_action["result"]["plan_receipt_included"])
            ...
            self.assertNotIn("send message relay controlled", json.dumps(relay_pull, sort_keys=True))
            self.assertNotIn("send message relay controlled", json.dumps(relay_action, sort_keys=True))
    ...
    def test_model_auth_subscription_login_is_guarded_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(doctor_checks["Azure Foundry"]["activation_state"], "config_required")
            self.assertIn("models.azure_foundry_base_url", doctor_checks["Azure Foundry"]["activation"]["missing_config"])
            ...
            doctor_checks_after = {row["target"]: row for row in doctor_after["auth_doctor"]["checks"]}
            self.assertIn("verified_ready", doctor_after["auth_doctor"]["activation_state_counts"])
            self.assertEqual(doctor_checks_after["Google Vertex AI / Gemini cloud identity"]["activation_state"], "verified_but_invocation_blocked")
            self.assertIn("models.google_vertex_project", doctor_checks_after["Google Vertex AI / Gemini cloud identity"]["activation"]["missing_config"])
