...
class ModelAuthTests(unittest.TestCase):
    ...
    def test_verified_provider_native_auth_links_update_targets_and_logout(self) -> None:
        with tempfile.TemporaryDirectory() as temp, patch.dict(os.environ, {}, clear=True):
            ...
            with patch("aegis.models.registry._open_auth_request", fake_open):
                login = registry.login_provider_external("github-copilot", method="oauth-device", run_external=True, timeout_seconds=5)
            qwen_oauth_login = registry.login_provider_external("qwen", method="oauth", run_external=True)
            ...
            self.assertEqual(qwen_oauth_login["status"], "provider_discontinued")
            self.assertFalse(qwen_oauth_login["external_login_attempted"])
            self.assertFalse(qwen_oauth_login["token_captured"])
            qwen_discontinued_guidance = " ".join(qwen_status["provider_native_auth"][0]["next_steps"])
            self.assertIn("model auth login qwen --subscription", qwen_discontinued_guidance)
            self.assertNotIn("model auth login qwen --method oauth --run-external", qwen_discontinued_guidance)
            ...
            self.assertEqual(by_target["Qwen OAuth (discontinued)"]["status"], "provider_discontinued")
            self.assertEqual(by_target["Qwen OAuth (discontinued)"]["bridge_status"], "provider_discontinued")
            self.assertIn("Qwen OAuth (discontinued)", targets["provider_discontinued_targets"])
            self.assertNotIn("Qwen OAuth (discontinued)", targets["subscription_bridge_targets"])
    ...
    def test_provider_auth_targets_track_hermes_and_claude_gaps(self) -> None:
        with tempfile.TemporaryDirectory() as temp, patch.dict(os.environ, {}, clear=True):
            ...
            self.assertEqual(targets["provider_discontinued_count"], 1)
            self.assertIn("Qwen OAuth (discontinued)", targets["provider_discontinued_targets"])
            self.assertNotIn("Qwen OAuth (discontinued)", targets["operator_login_required_targets"])
            ...
            self.assertEqual(by_target["Qwen OAuth (discontinued)"]["required_auth"], ["oauth"])
            self.assertEqual(by_target["Qwen OAuth (discontinued)"]["status"], "provider_discontinued")
            self.assertEqual(by_target["Qwen OAuth (discontinued)"]["bridge_status"], "provider_discontinued")
            self.assertEqual(by_target["Qwen OAuth (discontinued)"]["provider_token_source"], "official Qwen Code auth store")
            qwen_oauth_guidance = " ".join(by_target["Qwen OAuth (discontinued)"]["next_steps"])
            self.assertIn("model auth login qwen --subscription", qwen_oauth_guidance)
            self.assertNotIn("model auth login qwen --method oauth --run-external", qwen_oauth_guidance)
    ...
    def test_model_auth_readiness_packet_is_private_and_verifier_rejects_tampering(self) -> None:
        with tempfile.TemporaryDirectory() as temp, patch.dict(os.environ, {}, clear=True):
            ...
            self.assertEqual(created["receipt"]["provider_discontinued_count"], 1)
            ...
            self.assertEqual(payload["provider_discontinued_count"], 1)
            self.assertIn("Qwen OAuth (discontinued)", payload["provider_discontinued_targets"])
            self.assertEqual(payload["activation_state_counts"]["provider_discontinued"], 1)
            payload_checks = {row["target"]: row for row in payload["checks"]}
            self.assertEqual(payload_checks["Qwen OAuth (discontinued)"]["activation_state"], "provider_discontinued")
            self.assertEqual(payload_checks["Qwen OAuth (discontinued)"]["login_command"], "")
            self.assertEqual(payload_checks["Qwen OAuth (discontinued)"]["verify_command"], "")
